...
% Exact solution: (sin(x).^2-1)/(2*cos(x))
...
%Largest error occurs when x t is approximately 1.5s. and the error value
%is between 10^-4 and 10^-5. The largest error occurs around 1.5s because
%the y values are close to zero, meaning that a small difference is
%relatively large to the actual y-value. The slope is also the greatest
%there, meaning that a small uncertainty in t will result in the greatest
%uncertainty in y.
...
%There seems to be a vertical asymptote before x=1, which is why it becomes
%unintegrable.
...
phi=[sym(y0)];   % we will keep a list with all the approximations
...
for i = 1:N
    ...
    func=subs(func,t,s);      % variable of integration is s, so we need to change
                              % t -> s
    newphi = y0 + int(func, s, 0 ,t);   % integrate to find next approximation
    phi=cat(2,phi,[newphi]);       % update the list of approximations by adding new phi
end

% Show the 5th approximation
phi(5)
% Plot the approximation just found
picard=ezplot(phi(N+1),[0,1]);
set(picard,'Color','green');         % set the color of the graph to green
