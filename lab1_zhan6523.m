...
N = 5;             %Alter this value to see different results
h = (1-0)/(N-1);
...
for j = 1:N       %Creates the desired x vector
    x(j) = ((j-1) * h) - 3;
end
disp(x)
...
for i = 1:N       %Creates the desired y vector
    y(i) = abs(x(i))*x(i);
end  
...
N = 5000001
x = zeros(N,1);   %Preallocates the array
h = (3-2)/(N-1);
...
for j = 1:N       %Creates the desired y vector using for loop
    y(j) = atan(x(j));
end
toc;
...
clear y;
tic;
y = atan(x);      %Creates the desired y vector using vectorization
toc;
...
%General solution: y = (t + C)/e^(t^2)

y1 = (t - exp(1) + 1)./exp2 %First solution when y(1) = -1
y2 = (t - 1)./exp2 %y(1) = 0
y3 = (t + exp(1) - 1)./exp2 %y(1) = 1

plot(t,y1,t,y2,t,y3);
xlabel('t')
ylabel('y');
title('Plot of three solutions to the ODE dy/dt + (2*t)*y = 1/exp(t^2)');
legend('y1 = -1', 'y2 = 0','y3 = 1', 'Location','SouthEast');
...
% Construct an array of 100 grid points from -5 to 5, and plot f(x).
x = linspace(-5,5);
plot(x, f(x));
% Label axes
xlabel('x');
...
%As x approaches infinity (as well as negative infinity since the solution
%is symmetric), if the y-value to the solution is positive, then y also
%approaches infinity. If the solution's y-value is negative, then y
%approaches 0 from the negative side.
%Solutions are symmetric because in the general solution, y is a function
%of x^2, meaning that both plugging in x and -x will result in the same
%y-value.
%% Exercise 12
%
...
% The constant solutions that are approached as x tends to inifinity are:
% y=2, y=1, and y=-1.
%If y_0>2, then y --> infinity as x -->infinity. y --> 2 as x --> -infinity
%If 1<y_0<2, then y --> 1 as x --> infinity. y --> 2 as x --> -infinity
%If -1<y_0<1, then y --> 1 as x --> infinity. y --> -1 as x --> -infinity
%If y_0<-1, then y --> -1 as x --> infinity. y --> -infinity as x -->
%-infinity
